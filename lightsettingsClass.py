...
class lightsettingsClass:
    ...
    def save_lightprofiles(self):
        ...
        if returnvalue:
            ...
            self.print_on_GUI_terminal(text_to_print=str(RGB_valllll)+str(Brighdhfdf),  color='default')
            text_to_print = str(loaded_light_profile) + "; profile " + str(loaded_light_profile) + "; RGB;" + str(RGB_valllll) + "; BRIGHTNESS; [" + str(Brighdhfdf) + "] \n"
            lightsettingsClass.replace_line('/home/dgslr/ProgramFiles/LightProfiles.txt', loaded_light_profile + 2, text_to_print)  # Offset of 2 to start looking after initilisation of file
