...
class lightsettingsClass:
    ...
    def lightprofiles(self):
        ...
        if previous_key_val:
            ...
            lightInputs[0][1] = int(lines[int(profile.previous_key)+2][68])
            lightInputs[0][2] = int(lines[int(profile.previous_key)+2][70])
            lightInputs[1][0] = int(lines[int(profile.previous_key)+2][72])
            lightInputs[1][1] = int(lines[int(profile.previous_key)+2][74])
            lightInputs[1][2] = int(lines[int(profile.previous_key)+2][76])
            lightInputs[2][0] = int(lines[int(profile.previous_key)+2][78])
            lightInputs[2][1] = int(lines[int(profile.previous_key)+2][80])
            lightInputs[2][2] = int(lines[int(profile.previous_key)+2][82])

            self.w.check_Top_Enable.setChecked(lightInputs[0][0])
            self.w.check_Left_Enable.setChecked(lightInputs[1][0])
            self.w.check_Right_Enable.setChecked(lightInputs[2][0])
            self.w.check_Top_RGB.setChecked(lightInputs[0][1])
            self.w.check_Left_RGB.setChecked(lightInputs[1][1])
            self.w.check_Right_RGB.setChecked(lightInputs[2][1])
            self.w.check_Top_White.setChecked(lightInputs[0][2])
            self.w.check_Left_White.setChecked(lightInputs[1][2])
            self.w.check_Right_White.setChecked(lightInputs[2][2])


            self.print_on_GUI_terminal(text_to_print= "LIGHTMATRIX1 " + str(lightInputs[0][0]),  color='default')
            self.print_on_GUI_terminal(text_to_print= "LIGHTMATRIX2 " + str(lightInputs[1][0]),  color='default')
            self.print_on_GUI_terminal(text_to_print= "LIGHTMATRIX3 " + str(lightInputs[2][0]),  color='default')
            self.print_on_GUI_terminal(text_to_print= "LIGHTMATRIX4 " + str(lightInputs[0][1]),  color='default')
            self.print_on_GUI_terminal(text_to_print= "LIGHTMATRIX5 " + str(lightInputs[1][1]),  color='default')
            self.print_on_GUI_terminal(text_to_print= "LIGHTMATRIX6 " + str(lightInputs[2][1]),  color='default')
            self.print_on_GUI_terminal(text_to_print= "LIGHTMATRIX7 " + str(lightInputs[0][2]),  color='default')
            ...
            self.print_on_GUI_terminal(text_to_print= "LIGHTMATRIX9 " + str(lines[int(profile.previous_key)+2][55:82]),  color='default')
